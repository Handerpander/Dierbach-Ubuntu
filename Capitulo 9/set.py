...
def greeting():
    ...
    """Displays the progrma greeting on the screen. """

    print('This program will display the title pattern for a selected')
    print('pair of tile colors, tile size, and grout color.\n')
    print('if skip = 1, secondary title color placed every other title,')
...
def getTileSelections(grout_color_options):
    ...
    """"Returns dictionary of the form,
            ...
            {'title_size':dict, 'color1':str, 'color2':str, 'skip':int, 'grout':str}

            where the value of tile_size is a dictionary of the form
            {'title_length': value, 'title_width': value},
    ...
    title_width = int(input('Enter tile width (1-4 inches): '))
    while title_width < 1 or title_width > 4:
    ...
    skip = int(input('Enter frequency of seconday tile color (1-10): '))
    ...
    while (grout not in grout_color_options):
        grout = input('INVALID grout color section, please re-enter: ')

    # prepend hash sign to indicate RGB string
    color1 = '#' + color1
    color2 = '#' + color2

    # creaete dictionary of selections
    selections = {'tile_size':{'lenth':tile_length, 'width':tile_width},
...
def layoutTiles(window, selections, tile_area, scaling):
    ...
    while not terminate_layout:
        ...
        if the_turtle.xcor()<tile_area['width']:
            ...
            # check if need to switch to secondry tile color
            if skip_counter == 0:
            ...
            # palce current tile color at current turtle location
            the_turtle.stamp()
