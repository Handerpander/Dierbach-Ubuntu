# Calendar Year Program (Function Version)
...
def getYear():
    ...
    while (year < 1800 or year > 2099) and year != -1:
        ...
        year = int(input('Enter year (1-12): '))
...
def numDaysInMonth(month_num, leap_year):
    ...
    num_day_in_month = (31, 28, 31, 30, 31, 30, 31, 31, 30, 30, 31)

    # special check for Frebruary in leap year
    if (month_num == 2) and leap_year:
        num_days = 29
    else:
        num_days = num_day_in_month[month_num - 1]
    return num_days

def contructCalMonth(month_num, first_day, num_days_in_nmonth):
    ...
    calendar_month = ['' + format(mont_names[month_num - 1], '<20')]
    current_day = 1
    current_col = 1
    calendar_week = ''
    ...
    while current_col < starting_col:
        ...
        while current_day <= num_days_in_nmonth:

            # store day of month in field of lenght 3
        ...
        # if there is a final week, append to contructed month
        if calendar_week != empty_str:
            calendar_month = calendar_month + [calendar_week]

        return calendar_month

def contructCalYear(year):

    """"Returns a fromatted calendar year for display on the screen.
    ...
    # construct calendar from twelve contructed months
    for month_num in range(1, 13):
        num_day_in_month = numDaysInMonth(month_num, leap_year)

        calendar_year = calendar_year + \
                        contructCalMonth(month_num, first_day_of_month,
                                         num_day_in_month)
...
def displayCalendar(calendar_year):
    ...
    """"Displays a calendar_year on the scren three months across."""
            ...
            while lines_to_print:
                ...
                for k in range(month_index, month_index + 3):
                    if week < len(calendar_year[k]):
                        print(calendar_year[k][week-1], end='')
                    else: print(blank_week, end='')
